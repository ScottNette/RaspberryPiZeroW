...
def verifyState():
    ...
    if ((MstrCntrl.selectedDevice == MstrCntrl.AllowList[0]) or (MstrCntrl.emailUnlock == True) ):
        for ii in range(0, 3):
            if (MstrCntrl.emailUnlock):
                MstrCntrl.BTCntrl.checkSingle(MstrCntrl.selectedDevice)
            else:
                MstrCntrl.BTCntrl.checkSingle(MstrCntrl.AllowListAlt[0])

            print(MstrCntrl.BTCntrl.foundDevice)
            #print(MstrCntrl.AllowList[0])
            if (MstrCntrl.BTCntrl.foundDevice == MstrCntrl.AllowListAlt[0]):
